...
def import_train_data_file(filename: str):
    train_data = []
    with open(filename, "r") as file:
        ...
        for line in file:
            if line == "\n" or line == "":
                entity_expected = False
                train_data.append((sentence, {'entities': tuples}))
                tuples = []
                continue

            if not entity_expected:
                sentence = line
                entity_expected = True
            else:
                first, last, ent = line.split()
                tuples.append((int(first), int(last), ent))
...
"""
TRAIN_DATA = import_train_data_file("ner_train_data.txt")
...
"""
model_nlps = {model: spacy.load(model) for model in models}


def get_docs_from_text(model, text: str, slice_size: int, slicing_character: str):
    offset = 0
    docs = []
    try:
    ...
    while len(slice_text) > 0:
        ...
        if len(slice_text) >= 1000000:
            slicing_character = " "
            continue

        offset += len(slice_text)

        nlp = model_nlps[model]
...
INPUT_FILES = [(f"data/extracted_texts/AMZ{i}.txt", i) for i in range(1, 51)]
...
def store_docs(input_files: List[str], model: str):
    for input_file, index in input_files:
        ...
        print(f"entities: {len([ent for ent in docs[0].ents if ent.label_ == 'PER'])}")

        with open(f"data/entities_files/AMZ{index}_{model}.dat", "wb") as file:
            pickle.dump(docs, file)

        print(f"stored: data/entities_files/AMZ{index}_{model}.dat")


for model in models:
    store_docs(INPUT_FILES, model)
